...
void vertex() {
	...
	TANGENT = normalize(vec3(0.0, height(VERTEX.xz + vec2(0.0, 0.1), TIME) - height(VERTEX.xz + vec2(0.0, -0.1), TIME), 0.1));
	BINORMAL = normalize(vec3(0.1, height(VERTEX.xz + vec2(0.1, 0.0), TIME) - height(VERTEX.xz + vec2(-0.1, 0.0), TIME ), 0.0));
